...
const ActivityBuilderScreen: React.FC = () => {
  ...
  const { courseId } = useParams<{ courseId?: string; activityId?: string }>(); // Using courseId for courses
  const navigate = useNavigate();

  // Common fields
  const [contentCategory, setContentCategory] = useState<ContentCategory>('ShortActivity');
  const [title, setTitle] = useState(''); // Name for activity, title for course
  ...
  const [isPremium, setIsPremium] = useState(false); // For short activities or one-time course price
  ...
  useEffect(() => {
    if (courseId && context?.allCourses && context.teacherProfile) {
      ...
      if (existingCourse) {
        ...
        setDifficulty(existingCourse.lessons[0]?.contentType === LessonContentType.Game ? DifficultyLevel.Easy : DifficultyLevel.AllLevels); // Simplified
        ...
        setLessons(existingCourse.lessons.map(l => ({...l, title: l.title, description: l.description, contentType: l.contentType, content: l.content}))); // Ensure content is copied
      } else {
        // Could be a short activity if we passed activityId
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (contentCategory === 'LongCourse') {
        const courseData: Partial<Course> = {
            ...
            lessons: lessons.map((l, idx) => ({ // Ensure lessons have all required fields
            ...
            imageUrl: 'https://picsum.photos/seed/courseplaceholder/300/200', // Placeholder
        };
        console.log("Submitting Course:", courseData);
        // Mock API call for course: context.saveCourse(courseData);
        // For demo, we'll just log it and navigate away.
        // In a real app:
        // const savedCourse = await saveCourseToBackend(courseData); 
        // context.updateCoursesList(savedCourse); 
    } else { // ShortActivity
        const activityData: Partial<Activity> = {
            name: title,
            contentType: shortActivityContentType,
            activityContent: {...activityContentDetails, title, description}, // Add title and desc to activityContent
            ...
            creatorType: 'Teacher',
            status: ActivityStatus.Pending,
            icon: SparklesIcon, // Assign component type, not instance
            color: 'bg-gray-400', 
        };
        console.log("Submitting Short Activity:", activityData);
        // Mock API call for activity: context.saveActivity(activityData);
    }
                ...
                {lessons.map((lesson, index) => (
                  <Card key={index} className="!p-3 !shadow-sm">
                    ...
                    <select value={lesson.contentType || LessonContentType.Video} onChange={e => handleLessonChange(index, 'contentType', e.target.value as LessonContentType)} className="w-full px-2 py-1 border rounded mb-1 text-sm">
                        {Object.values(LessonContentType).map(type => <option key={type} value={type}>{type}</option>)}
                    </select>
                     {/* Basic content field based on type - very simplified */}
            ...
            <>
              <div>
                ...
                <select id="shortActivityContentType" value={shortActivityContentType} onChange={(e) => setShortActivityContentType(e.target.value as LessonContentType)} className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-teal-500 focus:border-teal-500 sm:text-sm rounded-md">
                  {Object.values(LessonContentType).filter(t => ![LessonContentType.LiveSessionLink].includes(t)).map(type => <option key={type} value={type}>{type.replace(/([A-Z])/g, ' $1').trim()}</option>)}
                </select>
              </div>
              <div>
                ...
                <select id="difficulty" value={difficulty} onChange={(e) => setDifficulty(e.target.value as DifficultyLevel)} className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-teal-500 focus:border-teal-500 sm:text-sm rounded-md">
                  {Object.values(DifficultyLevel).filter(l => l !== DifficultyLevel.AllLevels).map(level => <option key={level} value={level}>{level}</option>)}
                </select>
              </div>
               <div className="flex items-center">
              ...
              {/* Add more specific content fields for short activities here if needed */}
            </>
          )}
