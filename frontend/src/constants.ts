...
import { 
    ...
    ReceiptPercentIcon, BuildingLibraryIcon, CubeIcon, CpuChipIcon, RocketLaunchIcon // Added more icons
...
// Expanded ACTIVITY_CATEGORIES_CONFIG for V1 KidHomeScreen style V5
// IDs should match Activity['category'] values (case-insensitive) for easier filtering.
export const ACTIVITY_CATEGORIES_CONFIG: ActivityCategory[] = [
    { id: 'alphabet', name: 'ABC Fun', icon: AcademicCapIcon, color: 'bg-rose-400 hover:bg-rose-500' }, // Changed id from 'abc'
    { id: 'numbers', name: 'Numbers', icon: CalculatorIcon, color: 'bg-orange-400 hover:bg-orange-500' },
    { id: 'mathpuzzles', name: 'Math Puzzles', icon: LightBulbIcon, color: 'bg-sky-500 hover:bg-sky-600' }, // id matches 'MathPuzzles' category type
    { id: 'reading', name: 'Reading', icon: BookOpenIcon, color: 'bg-lime-500 hover:bg-lime-600' },
    { id: 'puzzles', name: 'Puzzles', icon: PuzzlePieceIcon, color: 'bg-green-500 hover:bg-green-600' }, // General puzzles
    { id: 'drawing', name: 'Drawing', icon: PaintBrushIcon, color: 'bg-purple-500 hover:bg-purple-600' },
    { id: 'science', name: 'Science Lab', icon: BeakerIcon, color: 'bg-teal-400 hover:bg-teal-500' },
    { id: 'space', name: 'Space', icon: GlobeAltIcon, color: 'bg-indigo-500 hover:bg-indigo-600' }, 
    { id: 'tech', name: 'Tech Time', icon: CpuChipIcon, color: 'bg-slate-500 hover:bg-slate-600' },
    // Removed duplicate number and reading categories, drawing, and puzzles for V1 style as they are covered by new ones.
    // The ones above are more thematic. Add 'general' if needed.
];
...
// Updated createActivities to use Activity['category']
const createActivities = (category: Activity['category'], baseName: string, icon: React.ElementType, color: string, count: number, startView: View = View.ActivityPlaceholder, contentType: LessonContentType = LessonContentType.Game): Activity[] => {
  const activities: Activity[] = [];
  for (let i = 1; i <= count; i++) {
    activities.push({
      id: `${category.toString().toLowerCase()}_${baseName.toLowerCase().replace(/\s+/g, '_')}_${i}`, // category.toString() for safety if it's an enum-like
      name: `${baseName} ${i}`,
      category,
      icon,
      color,
...
export const ACTIVITIES_CONFIG: Activity[] = [
  ...
  ...createActivities('Alphabet', 'Letter Match', AcademicCapIcon, 'bg-rose-400', 3, View.LetterSoundsScreen, LessonContentType.LetterSound),
  ...createActivities('Alphabet', 'Phonics Fun', MusicalNoteIcon, 'bg-rose-400', 2, View.ActivityPlaceholder, LessonContentType.Game),
  ...createActivities('Alphabet', 'Trace the Letter', PencilIcon, 'bg-rose-400', 3, View.TraceAnimalScreen, LessonContentType.DrawingChallenge), // Repurpose TraceAnimalScreen
  ...
  ...createActivities('Numbers', 'Number Recognition', MagnifyingGlassIcon, 'bg-orange-400', 3, View.ActivityPlaceholder, LessonContentType.NumberRecognition),
  ...createActivities('Numbers', 'Shape Counting', CubeIcon, 'bg-orange-400', 2), 
  ...createActivities('Numbers', 'Number Tracing', PencilIcon, 'bg-orange-400', 2, View.TraceAnimalScreen, LessonContentType.DrawingChallenge),

  // --- MATH PUZZLES CATEGORY --- Use category: 'MathPuzzles'
  { 
    id: 'simple_addition_1', name: 'Easy Addition', category: 'MathPuzzles', icon: LightBulbIcon, color: 'bg-sky-500', 
  ...
  ...createActivities('MathPuzzles', 'Number Patterns', LightBulbIcon, 'bg-sky-500', 3, View.ActivityPlaceholder, LessonContentType.PatternCompletion), 
  ...createActivities('MathPuzzles', 'Math Logic', AdjustmentsHorizontalIcon, 'bg-sky-500', 2, View.ActivityPlaceholder, LessonContentType.Puzzle), 

  // --- READING CATEGORY (Reading & Stories) ---
  { 
    id: 'fairytale_reader_1', name: 'Fox Learns to Read', category: 'Reading', icon: BookOpenIcon, color: 'bg-lime-500', 
  ...
  { 
    id: 'letter_sounds_1', name: 'Letter Sounds Fun (Advanced)', category: 'Reading', icon: MusicalNoteIcon, color: 'bg-lime-500', 
    view: View.LetterSoundsScreen, ageGroups: ['5-7'], contentType: LessonContentType.LetterSound,
  ...
  ...createActivities('Reading', 'Sight Words', EyeIcon, 'bg-lime-500', 3),
  ...createActivities('Reading', 'Story Sequencing', BuildingLibraryIcon, 'bg-lime-500', 2, View.ActivityPlaceholder, LessonContentType.Story),
  ...createActivities('Reading', 'Rhyming Words', ChatBubbleLeftRightIcon, 'bg-lime-500', 2),
  ...
  ...createActivities('Puzzles', 'Odd One Out', QuestionMarkCircleIcon, 'bg-green-500', 2, View.ActivityPlaceholder, LessonContentType.Puzzle),
  ...createActivities('Puzzles', 'Logic Grid', ReceiptPercentIcon, 'bg-green-500', 2),
  ...createActivities('Puzzles', 'Connect the Dots', LinkIcon, 'bg-green-500', 2, View.ActivityPlaceholder, LessonContentType.DrawingChallenge),
  ...
  ...createActivities('Drawing', 'Finish the Picture', LightBulbIcon, 'bg-purple-500', 2, View.ActivityPlaceholder, LessonContentType.DrawingChallenge),
  ...createActivities('Drawing', 'Doodle Prompts', SparklesIcon, 'bg-purple-500', 2, View.ActivityPlaceholder, LessonContentType.DrawingChallenge),
  ...
  // --- SCIENCE CATEGORY ---
  { 
    id: 'simple_experiments_1', name: 'Kitchen Science', category: 'Science', icon: BeakerIcon, color: 'bg-teal-400', 
    view: View.SimpleExperimentsScreen, ageGroups: ['5-7', '8-10'], contentType: LessonContentType.ScienceExperiment, 
    creatorType: 'System', difficulty: DifficultyLevel.Easy, status: ActivityStatus.Approved,
    activityContent: { title: "Float or Sink?", description: "Discover what floats and what sinks!" }
  },
  ...createActivities('Science', 'Nature Hunt', MagnifyingGlassIcon, 'bg-teal-400', 2, View.ActivityPlaceholder, LessonContentType.Game),
  ...createActivities('Science', 'Weather Watch', CloudIcon, 'bg-teal-400', 2, View.ActivityPlaceholder, LessonContentType.TextArticle),

  // --- SPACE CATEGORY ---
  ...createActivities('Space', 'Planet Match', GlobeAltIcon, 'bg-indigo-500', 3, View.MemoryMatchScreen, LessonContentType.MemoryGame),
  ...createActivities('Space', 'Draw an Alien', SparklesIcon, 'bg-indigo-500', 2, View.DrawAndTell, LessonContentType.DrawingChallenge),
  ...createActivities('Space', 'Star Constellations', SunIcon, 'bg-indigo-500', 2, View.ActivityPlaceholder, LessonContentType.TextArticle),
  
  // --- TECH CATEGORY ---
  ...createActivities('Tech', 'Computer Parts', CpuChipIcon, 'bg-slate-500', 2, View.ActivityPlaceholder, LessonContentType.InteractiveQuiz),
  ...createActivities('Tech', 'Build a Robot Puzzle', PuzzlePieceIcon, 'bg-slate-500', 2, View.JigsawPuzzleScreen, LessonContentType.Puzzle),
  ...createActivities('Tech', 'Simple Coding Logic', CodeBracketIcon, 'bg-slate-500', 2, View.ActivityPlaceholder, LessonContentType.CodingActivity),

  // --- GENERAL / OTHER ACTIVITIES (ensure they have a category if not covered above) ---
  { 
